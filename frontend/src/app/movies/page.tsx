...
import { Search, Film, TrendingUp, Library } from "lucide-react";
  ...
  useEffect(() => {
    async function fetchMovies() {
      ...
      try {
        let url: string;
        if (search.trim()) {
          url = `${API_BASE}/api/v1/media/search?q=${encodeURIComponent(search)}&media_type=movie`;
        } else if (view === "library") {
          url = `${API_BASE}/api/v1/media/library?media_type=movie`;
        } else {
          url = `${API_BASE}/api/v1/media/trending?media_type=movie`;
        }
